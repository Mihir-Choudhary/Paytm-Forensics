...
def main():
    ...
    if not os.path.isdir(case_dir):
        ap.error(f"case_dir is not a directory: {case_dir}")
    if not os.path.isfile(os.path.join(case_dir, "case.db")):
        ap.error(f"no case.db in {case_dir} — build a case first with:\n"
                 f"  python run_gui.py --extraction <folder> --out <case_dir>")
