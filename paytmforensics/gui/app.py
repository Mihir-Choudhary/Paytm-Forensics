...
from .datasource import DataSource, DOMAIN_LABELS, CaseOpenError
...
def _privacy_note() -> str:
    from ..core.privacy import summary_line
    return summary_line()
...
class MainWindow(QMainWindow):
    def __init__(self, case_dir: str):
        ...
        self._masked = False          # sensitive-data masking starts OFF
    ...
    def _build(self):
        ...
        self.mask_banner = QLabel("  🙉  " + _privacy_note())
        self.mask_banner.setObjectName("maskBanner")
        self.mask_banner.setWordWrap(True)
        self.mask_banner.setVisible(False)
        rv.addWidget(self.mask_banner)
    ...
    def _topbar(self) -> QWidget:
        ...
        self.mask_btn = QPushButton("🙈  Hide sensitive data")
        self.mask_btn.setCheckable(True)
        self.mask_btn.setToolTip(
            "Mask names, phone numbers, customer/device IDs, UPI IDs, account and IFSC\n"
            "identifiers, RRNs and precise coordinates — for screen-sharing and\n"
            "screenshots. Amounts, dates and provenance stay visible. The case database\n"
            "is never altered.")
        self.mask_btn.toggled.connect(self._toggle_mask)
        h.addWidget(self.mask_btn)
    ...
    def _filter_card(self) -> QWidget:
        ...
        self.preset_box = QComboBox(); self.preset_box.setMinimumWidth(150)
        self.preset_box.addItem("— presets —")
        self.preset_box.currentIndexChanged.connect(self._load_preset)
        save_btn = QPushButton("Save preset"); save_btn.clicked.connect(self._save_preset)
        self._presets_path = os.path.join(self.case_dir, "filter_presets.json")
        self._presets = self._read_presets()
        for nm in sorted(self._presets):
            self.preset_box.addItem(nm)
        ...
        g.addWidget(self.f_src, 2, 0, 1, 5)
        g.addWidget(self.preset_box, 2, 5); g.addWidget(save_btn, 2, 6)
        self.f_warn = QLabel(""); self.f_warn.setObjectName("filterWarn")
        self.f_warn.setWordWrap(True); self.f_warn.setVisible(False)
        g.addWidget(self.f_warn, 3, 0, 1, 8)
        return card
    ...
    def _save_preset(self):
        ...
        if not ok or not name.strip():
            return
        name = name.strip()
        self._presets[name] = self._current_spec().to_dict()
        try:
            with open(self._presets_path, "w", encoding="utf-8") as f:
                json.dump(self._presets, f, indent=2, sort_keys=True, ensure_ascii=False)
        except OSError as e:
            QMessageBox.warning(self, "Preset", f"Could not save: {e}"); return
        if self.preset_box.findText(name) < 0:
            self.preset_box.addItem(name)
        self.statusBar().showMessage(f"Saved filter preset '{name}'")

    def _load_preset(self, idx: int):
        if idx <= 0:
            return
        spec = FilterSpec.from_dict(self._presets.get(self.preset_box.itemText(idx), {}))
        self.f_text.setText(spec.text or "")
        self.f_from.setText(spec.date_from or ""); self.f_to.setText(spec.date_to or "")
        self.f_origin.setCurrentText(spec.origin or "any")
        self.f_amin.setText("" if spec.amount_min is None else str(spec.amount_min))
        self.f_amax.setText("" if spec.amount_max is None else str(spec.amount_max))
        self.f_dir.setCurrentText(spec.direction or "")
        self.f_src.setText(spec.source_contains or "")
        self._apply()
        ...
        for group_title, domains in NAV_GROUPS:
            ...
            for dom in domains:
                ...
                if dom == "dashboard":
                    ...
                    # show a known domain with (0) rather than hiding it: an examiner must
                    # be able to tell "parsed, none found" from "never attempted"
                    n = counts.get(dom, 0)
                    text = f"{glyph}   {DOMAIN_LABELS.get(dom, dom)}   ({n})"
                it = QListWidgetItem(text)
                it.setData(Qt.UserRole, dom)
                if dom != "dashboard" and not counts.get(dom):
                    from PySide6.QtGui import QColor
    ...
    def _on_nav(self, cur, _prev):
        ...
        if dom == "dashboard":
            ...
            self._model = None      # else JSON/CSV would export the last grid viewed
    ...
    def _toggle_mask(self, on: bool):
        """Turn presentation masking on/off and rebuild every view that caches text."""
        self._masked = bool(on)
        self.ds.set_mask(self._masked)
        self.mask_btn.setText("🙉  Sensitive data hidden" if on
                              else "🙈  Hide sensitive data")
        self.mask_btn.setObjectName("primary" if on else "")
        self.mask_btn.setStyleSheet(self.mask_btn.styleSheet())   # re-evaluate QSS
        self.mask_banner.setVisible(self._masked)
        # pages that bake text at construction time must be rebuilt
        ...
        self.statusBar().showMessage(
            "Sensitive data hidden (display only — case.db unchanged)" if on
    ...
    def _current_spec(self) -> FilterSpec:
        ...
        def fnum(w):
            ...
            try:
                ...
                bad.append(txt)     # silently ignoring this made the filter look applied
                return None
        spec = FilterSpec(
        ...
        from .filters import parse_bound
        for label, val in (("From", self.f_from.text().strip()),
                           ("To", self.f_to.text().strip())):
            if val and parse_bound(val, end=False) is None:
                bad.append(f"{label} date '{val}'")
        self._bad_input = bad
        return spec
    ...
    def _apply(self):
        ...
        bad = getattr(self, "_bad_input", None)
        # Surface invalid input INLINE, never in a modal: a modal blocks headless and
        # scripted use entirely, and nagging on every Apply is worse UX than a banner.
        if bad:
            self.f_warn.setText("⚠  ignored (not understood): " + ", ".join(bad)
                                + "   — use YYYY-MM-DD for dates, plain numbers for amounts")
            self.f_warn.setVisible(True)
        else:
            self.f_warn.clear()
            self.f_warn.setVisible(False)
        self._model.set_filter(spec)
        self.table.resizeColumnsToContents()
        msg = f"{self._model.rowCount()} rows after filter"
        if bad:
            msg += f"  ({len(bad)} value(s) ignored — see warning above)"
    ...
    def _on_row_click(self, index):
        if not self._model:
            return
        rec = self.ds.mask(self._model.record_at(index.row()))
    ...
    def _export(self, fmt: str):
        ...
        if self._masked:
            rows = [self.ds.mask(r) for r in rows]
        exporters.export(rows, path, fmt)
        QMessageBox.information(
            self, "Export",
            f"Wrote {len(rows)} records to:\n{path}"
            + ("\n\nSensitive fields are MASKED in this export." if self._masked
               else ""))
    ...
    def _geo(self, fmt: str):
        ...
        n = geo.export(os.path.join(self.case_dir, "case.db"), path, fmt,
                       mask_sensitive=self._masked)
        QMessageBox.information(
            self, "Locations",
            f"Exported {n} GPS fixes to:\n{path}"
            + ("\n\nCoordinates are BLURRED (sensitive-data masking is on)."
...
def launch(case_dir: str):
    ...
    except CaseOpenError as e:
        # a wrong folder is an ordinary mistake; show it, don't traceback (NFR-3)
        QMessageBox.critical(None, "Cannot open case", str(e))
        return 2
    win.show()
    return app.exec()
