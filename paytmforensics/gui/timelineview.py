...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QFrame, QLabel, QTableView,
...
def _events(ds: DataSource):
    out = []
    for r in ds.load_shown("timeline"):
        utc = r.get("utc_iso")
        if not utc:
            continue
        try:
...
class TimelineView(QWidget):
    def __init__(self, ds: DataSource):
        ...
        # Size to content first, then stretch the SUMMARY column rather than the last one.
        # Stretching the last section gave `ref_domain` 77% of the viewport while the
        # timestamp and summary were truncated.
        self.table.resizeColumnsToContents()
        hdr = self.table.horizontalHeader()
        hdr.setStretchLastSection(False)
        cols = self.table.model()._cols
        if "summary" in cols:
            hdr.setSectionResizeMode(cols.index("summary"), QHeaderView.Stretch)
        else:
            hdr.setStretchLastSection(True)
        # Qt's setSortingEnabled applies an implicit sort on column 0; make the initial
        # order explicit and CHRONOLOGICAL so the view agrees with the stored order, the
        # HTML report and the CSV export. The examiner can still click to reverse it.
        from PySide6.QtCore import Qt as _Qt
        self.table.sortByColumn(0, _Qt.AscendingOrder)
