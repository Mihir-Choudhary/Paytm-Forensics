...
class ChatView(QWidget):
    ...
    def _subject_sb(self):
        for p in self.ds.load_shown("person"):
            if p.get("is_subject"):
                return p.get("sendbird_id")
        return None

    def _group(self) -> dict:
        msgs = [m for m in self.ds.load_shown("message") if m.get("channel_url")]
    ...
    def _bubble(self, m: dict, outgoing: bool) -> QWidget:
        ...
        if amt:
            mt = (m.get("msg_type") or "").upper()
            _g, _c, slabel = self._status(m)
            if "REQUEST" in mt and "RESPONSE" not in mt:
                verb = "Payment requested"
            elif slabel in ("failed", "declined"):
                # never head a failed payment "Paid"
                verb = f"Payment {slabel}"
            else:
                verb = "Sent" if outgoing else "Received"
            head = QLabel(f"₹{amt:g}  ·  {verb}")
            head.setStyleSheet(f"color:{fg}; font-weight:700; font-size:14px;")
            bl.addWidget(head)
        # The stored text is Paytm's own wording, written from the RECIPIENT's point of
        # view ("Sent you ₹90" on a message the subject SENT). Quote it explicitly so it
        # is never read as the tool's own statement about direction of funds.
        if amt and content:
            content = f'app text: "{content}"'
