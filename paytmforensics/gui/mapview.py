...
def _fixes(ds: DataSource) -> list[dict]:
    out = []
    for r in ds.load_shown("location"):
    ...
    for r in ds.load_shown("diagnostic"):
        if r.get("latitude") is not None and r.get("longitude") is not None:
...
<script>
 ...
 // only draw a movement path when there are genuinely distinct positions
 var uniq = {{}}; pts.forEach(function(p){{ uniq[p[0].toFixed(5)+','+p[1].toFixed(5)]=1; }});
...
class MapView(QWidget):
    def __init__(self, ds: DataSource):
        ...
        self._distinct = len({(round(f["lat"], 5), round(f["lon"], 5)) for f in self.fixes})
        legend = "   ".join(f"● {k} ({v})" for k, v in srcs.items())
        head = QLabel(f"Location map — {len(self.fixes)} fixes at {self._distinct} distinct "
                      f"position{'s' if self._distinct != 1 else ''}      {legend}")
        ...
        if getattr(self, "_is_web", False):
            note_txt = ("Interactive map (basemap tiles fetched online for display; evidence "
                        "parsing is fully offline). Dashed line = movement path.")
        else:
