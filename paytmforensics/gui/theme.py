...
DARK = {
    ...
    "border_soft":   "#1f2630",
    "accent":        "#2f81f7",
    "accent_on":     "#1550a0",   # accent as a TEXT background: >=4.5:1 against white
    "accent_cyan":   "#00b9f5",
    "text":          "#e6edf3",
    "text_muted":    "#8b949e",
    "text_dim":      "#9aa4ae",   # was #6e7681 -> 3.48:1 on cards (below AA)
    "green":         "#3fb950",
    "red":           "#f85149",
    "amber":         "#d29922",
    "purple":        "#a371f7",
...
LIGHT = {
    ...
    "text":          "#1f2328",
    "text_muted":    "#57606a",
    "text_dim":      "#6b737c",   # was #8b949e -> 3.08:1 on white (below AA)
    "green":         "#1a7f37",
    "red":           "#cf222e",
    "amber":         "#9a6700",
    "purple":        "#8250df",
...
def apply_palette(app) -> None:
    ...
    p.setColor(QPalette.Highlight, QColor(C.get("accent_on", C["accent"])))
...
NAV_GROUPS = [
    ("OVERVIEW", ["dashboard"]),
    ("FINANCIAL", ["transaction", "account", "entity", "carved"]),
    ("COMMUNICATIONS", ["message", "channel", "person"]),
...
QListWidget#nav::item:selected {{ background: {c['accent_on']}; color: white; font-weight: 600; }}
...
QPushButton:pressed {{ background: {c['accent_on']}; color: white; }}
QPushButton#primary {{ background: {c['accent_on']}; border: none; color: white; font-weight: 600; }}
...
QTableView {{ background: {c['bg_panel']}; alternate-background-color: {c['row_alt']};
    ...
    selection-background-color: {c['accent_on']}; selection-color: #ffffff; }}
QTableView::item {{ padding: 6px 8px; border: none; }}
/* explicit item-level rule: reliably colours the selected row in both themes */
QTableView::item:selected {{ background: {c['accent_on']}; color: #ffffff; }}
QTableView::item:selected:!active {{ background: {c['accent_on']}; color: #ffffff; }}
...
QMenu::item:selected {{ background: {c['accent_on']}; color: #ffffff; }}
QStatusBar {{ background: {c['bg_sidebar']}; color: {c['text_muted']}; border-top: 1px solid {c['border_soft']}; }}
QLabel#maskBanner {{ background: {c['chip']}; color: {c['amber']};
    border-bottom: 1px solid {c['border']}; padding: 6px 24px; font-size: 11px; }}
QLabel#filterWarn {{ color: {c['amber']}; font-size: 11px; padding: 4px 2px 0 2px; }}
