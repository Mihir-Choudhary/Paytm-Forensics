...
#: chat payment statuses that mean money actually moved. Matching only "SUCCESS" left
#: COMPLETED UPI requests out of every money total.
_SETTLED_STATUS = {"SUCCESS", "COMPLETED"}
    ...
    def parse(self) -> Iterator[Record]:
        ...
        with self.open(art) as con:
            ...
            for rowid, r in sql.rows(con, "ChatMessageEntity"):
                ...
                # A request that reports COMPLETED (and carries an RRN) is a real,
                # NPCI-referenced money movement -- previously it was direction=None and
                # settled=False, so it appeared in neither the Received nor Paid total.
                completed = status in _SETTLED_STATUS and bool(d.get("rrn"))
                if is_request and not completed:
                    direction = None               # an outstanding request; no money moved
                elif is_request and completed:
                    # the subject RAISED the request, so the counterparty paid THEM
                    direction = "credit" if (subject_sb and r.get("senderId") == subject_sb) \
                        else "debit"
                ...
                settled = (status in _SETTLED_STATUS) and "FAIL" not in ctype \
                    and "DECLIN" not in ctype
...
# NOTE: chat payments ARE emitted as `transaction` records by ChatLedgerParser above,
# deduped against the passbook on uniqueKey == sourceTxnId (passbook wins). They are ALSO
# emitted as `message` records by chats.py, deliberately: the message preserves the
# conversation context. The master timeline de-duplicates by skipping messages that carry
# an amount, so a payment appears exactly once chronologically.
