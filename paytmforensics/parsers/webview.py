    ...
    def parse(self) -> Iterator[Record]:
        ...
        for art in self._entries:
            ...
            yield WebCacheEntry(
                ...
                user_id=uid,
                size=len(data),
                body_preview=redact_text(json.dumps(obj, ensure_ascii=False))[:400],
    ...
    def parse(self) -> Iterator[Record]:
        for art in self._ldb_arts:
            ...
            for m in _LS_KEY.finditer(data):
                ...
                value = redact_text(vm.group(0).decode("ascii", "replace")) if vm else None
                sig = (origin, key)
                if sig in seen:
                    continue
                seen.add(sig)
