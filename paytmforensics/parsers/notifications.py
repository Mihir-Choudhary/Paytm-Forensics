        ...
        with self.open(art) as con:
            ...
            if "PushData" in tables:
                for rowid, r in sql.rows(con, "PushData"):
                    # `expiry` is a future deadline, not when anything happened. Putting
                    # it in `timestamp` put notifications a week in the future.
                    yield Notification(
                        provenance=self.prov(art, "PushData", rowid),
                        raw=r,
                        push_id=r.get("pushIdentifier"),
                        timestamp=None,
                        expiry=timestamps.decode(r.get("expiry")).to_dict(),
                        is_dedup_record=True,
