                ...
                if "TBL_USERS" in sql.list_tables(con):
                    for _rid, r in sql.rows(con, "TBL_USERS"):
                        if str(r.get("isMe")) == "1":
                            ...
                            # FR-1 wants the subject's account age; the extraction logic
                            # already existed for counterparties but was never called here
                            from .contacts import _account_age
                            account_age = _account_age(
                                r.get("getInfoSync") or r.get("getPaymentInfoSync"))
                            break
