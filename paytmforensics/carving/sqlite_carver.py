...
class SqliteCarver:
    def __init__(self, path: str = None, *, data: bytes = None, page_size: int = 0):
        if data is None:
            with open(path, "rb") as f:
                data = f.read()
        self.data = data
        self.page_size = page_size or self._page_size()
...
class WalCarver(SqliteCarver):
    """Carve a SQLite -wal file.

    A write-ahead log is a sequence of frames, each a 24-byte header followed by one full
    page image as it existed at some commit. Those page images are the richest source of
    recently-deleted rows on a live-extracted device -- the main database file may have
    been checkpointed clean while the WAL still holds the pre-delete pages.

    We apply the SAME unallocated-region logic per frame page (freeblocks + the gap
    between the cell-pointer array and the cell-content area), so live cells in a frame
    are never scanned and live data is never re-reported as carved.
    """

    def __init__(self, path: str):
        with open(path, "rb") as f:
            raw = f.read()
        if len(raw) < WAL_HEADER or raw[:4] not in (b"\x37\x7f\x06\x82", b"\x37\x7f\x06\x83"):
            super().__init__(data=b"", page_size=0)
            self.frames = []
            return
        page_size = int.from_bytes(raw[8:12], "big")
        super().__init__(data=raw, page_size=page_size if 512 <= page_size <= 65536 else 0)
        self.frames = []
        if not self.page_size:
            return
        off = WAL_HEADER
        stride = WAL_FRAME_HEADER + self.page_size
        while off + stride <= len(raw):
            pgno = int.from_bytes(raw[off:off + 4], "big")
            self.frames.append((pgno, raw[off + WAL_FRAME_HEADER:off + stride]))
            off += stride
        self.npages = len(self.frames)

    def _unallocated_regions(self):
        """Yield (page_no, region_kind, bytes) for every frame's page image."""
        for pgno, page in self.frames:
            hdr_off = 100 if pgno == 1 else 0
            if hdr_off >= len(page):
                continue
            if page[hdr_off] not in (0x0D, 0x0A):   # only leaf pages carry records
                continue
            try:
                ncells = int.from_bytes(page[hdr_off + 3:hdr_off + 5], "big")
                content_start = int.from_bytes(page[hdr_off + 5:hdr_off + 7], "big") or self.page_size
            ...
            cell_ptr_arr_end = hdr_off + 8 + ncells * 2
            if 0 < cell_ptr_arr_end < content_start <= len(page):
                yield pgno, "wal_gap", page[cell_ptr_arr_end:content_start]
            fb, guard = first_free, 0
            while 0 < fb < len(page) - 3 and guard < 256:
                nxt = int.from_bytes(page[fb:fb + 2], "big")
                size = int.from_bytes(page[fb + 2:fb + 4], "big")
                if size <= 0 or fb + size > len(page):
                    break
                yield pgno, "wal_freeblock", page[fb:fb + size]
                fb = nxt
                guard += 1
