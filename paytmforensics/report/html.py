...
REPORT_DOMAINS = ["entity", "person", "account", "transaction", "channel", "message",
                  "location", "consent",
...
#: default cap on rows rendered per table. The CLI passed None, so every config/pref/
#: diagnostic row was inlined into one self-contained HTML file (2.6 MB on a small case).
...
def _charts(ds: DataSource) -> str:
    # charts aggregate amounts/dates only, which are never masked
...
def _table(ds: DataSource, domain: str, limit: int | None = None) -> str:
    ...
    for r in shown:
        ...
        p = r.get("provenance", {})
        loc = p.get("rowid")
        loc = f"row {loc}" if loc is not None else (
            f"@{p.get('byte_offset')}" if p.get("byte_offset") is not None else "")
        sha = p.get("ingest_sha256") or ""
        # METHODOLOGY §2 promises reports surface rowid/byte_offset/confidence/hash
        out.append(f"<td class='prov'>{_esc(p.get('source_file'))}"
                   f"{(' :: ' + _esc(p.get('source_table'))) if p.get('source_table') else ''}"
                   f"{(' ' + _esc(loc)) if loc else ''}"
                   f"{('<br>sha256 ' + _esc(sha[:16]) + '…') if sha else ''}"
                   f"{('<br>read: ' + _esc(p.get('read_mode'))) if p.get('read_mode') else ''}</td>")
        out.append(f"<td class='origin-{_esc(p.get('origin'))}'>{_esc(p.get('origin'))}"
                   f"<br><span class='prov'>conf {_esc(p.get('confidence'))}</span></td>")
        out.append("</tr>")
...
def build_html(case_dir: str, *, table_limit: int | None = None,
               mask_sensitive: bool = False) -> str:
    meta = _load_json(os.path.join(case_dir, "case_meta.json"))
    manifest = _load_json(os.path.join(case_dir, "manifest.json"))
    ds = DataSource(os.path.join(case_dir, "case.db"), mask_sensitive=mask_sensitive)
    counts = ds.domains()
...
<h2>Summary</h2><div class="summary">"""]
    if mask_sensitive:
        ...
        parts.insert(1, f'<div class="warn"><b>REDACTED COPY.</b> {_esc(summary_line())}</div>')
    wal = _load_json(os.path.join(case_dir, "wal_report.json"))
    for dom in REPORT_DOMAINS:
        if dom in counts:
            parts.append(f"<span>{_esc(DOMAIN_LABELS.get(dom, dom))}: <b>{counts[dom]}</b></span>")
    parts.append("</div>")

    if wal.get("databases"):
        parts.append(
            "<h2>Write-ahead log (WAL) exposure</h2>"
            "<div class='warn'><b>%d database(s) carried a populated <code>-wal</code>.</b> "
            "Net %d row(s) exist only in a write-ahead log and would be invisible to an "
            "immutable-only read; net %d row(s) would be over-reported as live by such a "
            "read. This case read those databases from a verified copy with the WAL "
            "applied, so the table below reflects the true current state. Figures are net "
            "per-table row-count differences, not a row-level diff.</div>"
            % (len(wal["databases"]), wal.get("net_hidden_rows", 0),
               wal.get("net_deleted_rows", 0)))
        parts.append("<table><thead><tr><th>database</th><th>table</th>"
                     "<th>immutable view</th><th>with WAL applied</th></tr></thead><tbody>")
        for db, info in sorted(wal["databases"].items()):
            for tbl, c2 in sorted(info.get("tables", {}).items()):
                parts.append(f"<tr><td class='prov'>{_esc(db)}</td><td>{_esc(tbl)}</td>"
                             f"<td>{_esc(c2['immutable_view'])}</td>"
                             f"<td><b>{_esc(c2['with_wal'])}</b></td></tr>")
...
def generate(case_dir: str, out_path: str, fmt: str = "html",
    ...
    doc = build_html(case_dir, table_limit=table_limit, mask_sensitive=mask_sensitive)
    if fmt == "html":
